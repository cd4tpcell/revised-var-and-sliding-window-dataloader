...
def get_2d_sincos_pos_embed(embed_dim, grid_sizeh,grid_sizew, cls_token=False, extra_tokens=0):
    """
...
class PatchEmbedr(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, patch_size=8, in_chans=3,num_patches=8*8):
        super().__init__()
        self.proj = nn.Conv2d(in_chans,in_chans, kernel_size=patch_size, stride=patch_size)
...
class AttnBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.C = in_channels
        self.patch_size=8
        self.patchfy=PatchEmbedr(patch_size=self.patch_size, in_chans=self.C)

        self.norm = Normalize(in_channels)
        self.qkv = torch.nn.Conv2d(in_channels, 3*in_channels, kernel_size=1, stride=1, padding=0)
        self.w_ratio = int(in_channels) ** (-0.5)

        self.att_out = torch.nn.Conv2d(in_channels, in_channels*self.patch_size*self.patch_size, kernel_size=1, stride=1, padding=0)



        self.proj_out = torch.nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)

        self.proj_outx = torch.nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x0):
        ...
        pos_embed = torch.tensor(get_2d_sincos_pos_embed(embed_dim=self.C, grid_sizeh=h // patch_size[0],grid_sizew=w// patch_size[1])).float()

        x=self.patchfy(x0)+ pos_embed.reshape(1,c,h//8,w//8).to(x0.device)
...
class Encoder(nn.Module):
    def __init__(
        ...
        print(self.num_res_blocks,ch_mult,self.ch)
        # exit()
        self.in_channels = in_channels
        ...
        # self.norm_in = nn.BatchNorm2d(self.ch)
