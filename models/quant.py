...
class VectorQuantizer2(nn.Module):
    ...
    def get_minidx(self, en_out: torch.Tensor):

        B, C, H, W = en_out.shape

        
        en_out=en_out.permute(0, 2, 3, 1).reshape(-1, C)

        d_no_grad = torch.sum(en_out.square(), dim=1, keepdim=True) + torch.sum(self.embedding.weight.data.square(), dim=1, keepdim=False)
        d_no_grad.addmm_(en_out, self.embedding.weight.data.T, alpha=-2, beta=1)  # (B*h*w, vocab_size)
        idx_N = torch.argmin(d_no_grad, dim=1)
